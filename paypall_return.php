		...
		while (!feof($fp)) {
			...
			if (strcmp ($res, "VERIFIED") == 0) {
				if($_status == 'Completed'){
					...
					$query	  =	"Update tbl_order set orderstatus = 'c' where id = $_orderid";
										
					
				}
				else {
						$query	=	"Update tbl_order set status = 'p'  where id = $_orderid";
				}
			}
			
			else if (strcmp ($res, "INVALID") == 0){
				
				$query	= "Update tbl_order set orderstatus = 'p' where id = $_orderid";
			}
		}
		fclose($fp);
